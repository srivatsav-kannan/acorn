...
export const PlanPage = ({ workspace, catalog, onCommand }: { workspace: WorkspaceState, catalog: Catalog, onCommand: (command: Record<string, unknown>) => void | Promise<void> }) => {
  ...
  const plannedCourseIds = new Set(scenario?.courses.map((item) => item.courseId) ?? [])
  const addMatches = useMemo(() => addCourseOpen && addQuery.trim() ? searchCourses(catalog, { query: addQuery }).slice(0, 8) : [], [addCourseOpen, addQuery, catalog])
  const addCourseToTerm = async (courseId: string, code: string, units: number) => {
    if (!plan || !scenario) return
    const termSection = catalog.sections.find((item) => item.courseId === courseId && item.termId === plan.termId)
    await onCommand({ type: "edit_plan", planId: plan.id, scenarioId: scenario.id, operations: [{ type: "add_course", planCourse: { id: `PLANCOURSE-${code.replaceAll(" ", "-")}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, courseId, sectionId: termSection?.id ?? null, units, status: "active" } }] })
  }
  ...
  return <div className="page plan-page">
    ...
    <div className={`plan-layout ${termHasSchedule ? "" : "no-schedule"}`}>
      <section className="plan-course-panel" aria-label="Selected courses"><div className="section-heading"><div><h2>Courses</h2><span className="count-badge">{active.length}</span></div><strong>{units} units</strong></div>
        ...
        <button className="add-course-button" type="button" onClick={() => { setAddQuery(""); setAddCourseOpen(true) }}>+ Add course</button>
      ...
      {termHasSchedule ? <section className={`calendar-panel ${calendarMode === "list" ? "list-mode" : ""}`}><div className="section-heading"><div><h2>Weekly calendar</h2><span className="week-note">Pacific time</span></div><div className="calendar-toggle"><button className={calendarMode === "week" ? "active" : ""} aria-pressed={calendarMode === "week"} onClick={() => setCalendarMode("week")}>Week</button><button className={calendarMode === "list" ? "active" : ""} aria-pressed={calendarMode === "list"} onClick={() => setCalendarMode("list")}>List</button></div></div>
        ...
        <div className="section-heading"><div><h2>{termLabel(selectedTermId)}</h2><span className="week-note">{standingForTerm(timeline, selectedTermId)} year</span></div><button className="text-button" type="button" onClick={() => { setAddQuery(""); setAddCourseOpen(true) }}>Add course</button></div>
    ...
    {addCourseOpen && <div className="modal-backdrop" role="presentation" onMouseDown={() => setAddCourseOpen(false)}><section className="course-editor add-course-modal" role="dialog" aria-modal="true" aria-labelledby="add-course-title" onMouseDown={(event) => event.stopPropagation()}><div className="drawer-heading"><div><h2 id="add-course-title">Add to {termLabel(selectedTermId)}</h2></div><button className="icon-button" type="button" onClick={() => setAddCourseOpen(false)} aria-label="Close course search">×</button></div>
      <label className="add-course-search">Search the catalog<input autoFocus aria-label="Search the catalog" value={addQuery} onChange={(event) => setAddQuery(event.target.value)} /></label>
      {addQuery.trim() === "" ? <p className="add-course-hint">Search by course code or topic. Times attach automatically when the quarter has a stored schedule.</p> : addMatches.length === 0 ? <p className="add-course-hint">No catalog match. The Stanford tab has filters, and your agent can add missing reference courses with a source.</p> : <ul className="add-course-results">{addMatches.map(({ course, sections: courseSections }) => { const inPlan = plannedCourseIds.has(course.id); const termSection = courseSections.find((item) => item.termId === plan?.termId); return <li key={course.id}><div><b>{course.code}</b><small>{course.title}</small><span>{course.minUnits === course.maxUnits ? `${course.maxUnits} units` : `${course.minUnits} to ${course.maxUnits} units`}{termSection ? ` · ${termSection.meetings.map((meeting) => `${meeting.days.map((day) => day[0].toUpperCase()).join("")} ${meeting.start}`).join(", ")}` : " · times unconfirmed"}</span></div><button className={inPlan ? "secondary-button small" : "primary-button small"} type="button" disabled={inPlan} aria-label={`${inPlan ? "Added" : "Add"} ${course.code}`} onClick={() => void addCourseToTerm(course.id, course.code, course.maxUnits)}>{inPlan ? "Added" : "Add"}</button></li> })}</ul>}
    </section></div>}
    {commitmentOpen && <div className="modal-backdrop" role="presentation" onMouseDown={() => setCommitmentOpen(false)}><form className="course-editor" onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); void addCommitment() }}><div className="drawer-heading"><div><h2>Add a commitment</h2></div><button className="icon-button" type="button" onClick={() => setCommitmentOpen(false)} aria-label="Close commitment form">×</button></div><label>What is it?<input value={commitmentTitle} onChange={(event) => setCommitmentTitle(event.target.value)} required /></label><label>Day<select value={commitmentDay} onChange={(event) => setCommitmentDay(event.target.value as (typeof days)[number])}>{days.map((day) => <option value={day} key={day}>{day[0].toUpperCase() + day.slice(1)}</option>)}</select></label><div className="settings-inline"><label>Starts<input type="time" value={commitmentStart} onChange={(event) => setCommitmentStart(event.target.value)} /></label><label>Ends<input type="time" value={commitmentEnd} onChange={(event) => setCommitmentEnd(event.target.value)} /></label></div><div className="modal-actions"><button className="secondary-button" type="button" onClick={() => setCommitmentOpen(false)}>Cancel</button><button className="primary-button" type="submit">Add commitment</button></div></form></div>}
  </div>
}
