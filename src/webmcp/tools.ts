...
import { activeCourses, evaluateDegreePlan } from "@/domain/planning/degree-plan"
import { effectiveCompletedCourseIds } from "@/domain/workspace/history"
import { checkPlan, meetingComponent } from "@/domain/planning/planner"
import { suggestSections } from "@/domain/planning/scheduler"
import { mergedCatalogFor, mergedOpportunities } from "@/domain/workspace/reference"
import { nextMilestone, structuredGoals } from "@/domain/workspace/goals"
import { standingForTerm, supportsTimeline, termSequence, timelineFor } from "@/domain/planning/timeline"
import { institutionForWorkspace } from "@/data/institutions/registry"
import { evaluateRequirement } from "@/domain/planning/requirements"
import { searchCourses, searchWorkspace } from "@/domain/workspace/search"
import { exportBlocks } from "@/webmcp/export"
import type { Actor, WorkspaceState } from "@/domain/workspace/types"
...
export const createAcornTools = ({ repository, session, now, onWorkspaceChanged, runExclusive }: Setup): Tool[] => {
  ...
  return [
      ...
      inputSchema: schema({
        ...
        profile: {
          ...
          properties: {
            ...
            protectedWindows: { type: "array", description: "Up to four windows; check_plan warns when a section overlaps one", items: { type: "object", additionalProperties: false, properties: { id: { type: "string", description: "Stable ID; defaults by position" }, days: { type: "array", items: { type: "string", enum: ["mon", "tue", "wed", "thu", "fri", "sat", "sun"] }, description: "Days the window covers" }, start: { type: "string", description: "24h HH:MM" }, end: { type: "string", description: "24h HH:MM, after start" }, label: { type: "string", description: "Shown in warnings, 60 characters at most" } }, required: ["days", "start", "end"] } }
          }
        },
        academicHistory: {
          type: "object",
    ...
    {
      name: "set_interest",
      description: "Mark or unmark a course or a club as interesting. Interested clubs put their deadlines on the calendar; interested courses stay visible in the tracker until they are planned. Record an intended term with intendedTermId; keep free-form reasoning with annotate_course.",
    ...
    },
    {
      ...
      description: "Create or update a club membership or an ongoing commitment. Kind club with an opportunityId marks a joined directory club; its schedule and dated events show on the calendar under the club's name. A schedule of days and HH:MM times recurs between startDate and endDate; dates lists one-off events, optionally timed with start and end. Notes hold what to remember, and units count into the academics totals. Updates replace the whole activity, so send the full object. removeActivityId deletes.",
      inputSchema: schema({ expectedVersion: field("number", "Current workspace version"), idempotencyKey: field("string", "Unique retry-safe operation key"), activity: { type: "object", additionalProperties: false, description: "The activity to create or update; reuse an ID to update", properties: { id: field("string", "Stable ID; omit to create"), name: field("string", "Activity name"), kind: { type: "string", enum: ["club", "research", "job", "volunteering", "athletics", "arts", "other"], description: "Activity kind" }, detail: field("string", "What the work is"), notes: field("string", "Free notes kept with this activity"), units: field("number", "Registrar units this grants, if any, 1 to 10"), opportunityId: field("string", "Directory opportunity this club joins"), organizer: field("string", "Professor, group, or employer"), sourceUrl: field("string", "Related URL"), schedule: { type: "object", additionalProperties: false, description: "Recurring weekly block", properties: { days: field("array", "Days such as [\"tue\",\"thu\"]"), start: field("string", "HH:MM"), end: field("string", "HH:MM"), location: field("string", "Where") }, required: ["days", "start", "end"] }, startDate: field("string", "YYYY-MM-DD recurrence start"), endDate: field("string", "YYYY-MM-DD recurrence end"), dates: field("array", "One-off events as {date, label, start?, end?} with HH:MM times") }, required: ["name"] }, removeActivityId: field("string", "Activity ID to remove instead") }, ["expectedVersion", "idempotencyKey"]),
      annotations: annotations(false),
      examples: [],
    ...
    },
    {
      ...
      inputSchema: schema({
        expectedVersion: field("number", "Current workspace version"),
        idempotencyKey: field("string", "Unique retry-safe operation key"),
        action: { type: "string", enum: ["upsert", "toggle_milestone", "set_status", "remove"], description: "What to do; remove deletes a goal and its linked todos" },
        goal: {
          type: "object",
          additionalProperties: false,
        ...
        goalId: field("string", "For toggle_milestone, set_status, and remove"),
