...
export default function Page() {
  ...
  return <div className="page settings-page">
    ...
    <section className="settings-card account-card"><div><span className="type-icon">@</span><p><strong>{value.isDemoAccount ? "Demo account" : value.mode === "account" ? "Authenticated account" : value.localAccount ? "Browser workspace" : "Demo workspace"}</strong><small>{value.mode === "account" ? value.userEmail : value.localAccount ? "Stored in this browser. Resetting starts onboarding over." : "Seeded demo state for trying the product."}</small></p></div><span className={`status-pill ${value.mode === "account" ? "completed" : "planned"}`}>{value.mode === "account" ? "Saved to server" : "On this device"}</span></section>
