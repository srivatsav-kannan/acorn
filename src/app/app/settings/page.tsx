...
export default function Page() {
  ...
  return <><div className="page settings-page">
    <header className="page-heading"><div><BackButton /><h1>Settings</h1></div>{value.isDemoAccount || value.mode === "fixture" ? <button className="secondary-button" onClick={value.reset}>Reset demo to onboarding</button> : <button className="secondary-button" onClick={value.signOut}>Sign out</button>}</header>
    ...
    <section className="settings-card credit-settings"><div className="section-heading"><div><h2>AP and transfer credit</h2><span className="count-badge">{apCredits.length}</span></div><button className="secondary-button" type="button" onClick={() => setAddingCredit((current) => !current)}>{addingCredit ? "Cancel" : "Add credit"}</button></div>
      ...
      {addingCredit && <form className="credit-form" onSubmit={(event) => { event.preventDefault(); void addCredit() }}>
        <label>Exam or credit source<input value={creditExam} onChange={(event) => setCreditExam(event.target.value)} required maxLength={80} /></label>
        <div className="settings-inline">
          <label>Score<input value={creditScore} onChange={(event) => setCreditScore(event.target.value)} inputMode="numeric" maxLength={1} /></label>
          <label>Units granted<input value={creditUnits} onChange={(event) => setCreditUnits(event.target.value)} inputMode="numeric" maxLength={2} /></label>
        </div>
        <label>Counts as<input value={creditEquivalentId ? (value.catalog.courses.find((course) => course.id === creditEquivalentId)?.code ?? creditEquivalentQuery) : creditEquivalentQuery} onChange={(event) => { setCreditEquivalentQuery(event.target.value); setCreditEquivalentId("") }} placeholder="Search the catalog" /></label>
